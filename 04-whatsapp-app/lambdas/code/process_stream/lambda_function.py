...
from utils import (build_response,whats_reply)
            ...
            if event_name == "INSERT":
                ...
                for change in entry['changes']:
                    ...
                    if(messageType == 'text'):
                        ...
                        text = "I am currently not able to process images."
                        whats_reply(whatsapp_out_lambda,phone, WHATS_TOKEN, phone_id, text, messages_id)
...
def process_text(whats_message, whats_token,phone,phone_id,messages_id):
    ...
    print("text", whats_message, whats_token)
    ENV_AGENT_BEDROCK = os.environ['ENV_AGENT_BEDROCK']
    
    try:       

        response_3 = lambda_client.invoke(
            FunctionName = ENV_AGENT_BEDROCK,
            InvocationType = 'Event' ,#'RequestResponse', 
            Payload = json.dumps({
                'whats_message': whats_message,
                'whats_token': whats_token,
        ...
        error = err
        print(err.get("Error", {}).get("Code"))
        return f"Un error invocando {ENV_AGENT_BEDROCK}"
